...
/**
 * One entry per public page. `slug` matches the URL path AND the template files
 * under server/render/templates/{slug}.head.html / .tail.html. `file` (optional)
 * points to a source HTML file to auto-seed sections from on first boot — useful
 * when migrating an existing static site. Leave blank for an empty starter page.
 */
...
export function seedInitialContent(opts: SeedOptions = {}): SeedResult {
  ...
  for (const { slug, file } of PAGES) {
    ...
    if (!file) continue; // empty page — admin will add sections via the UI
